...
from core.pipeline_qwen.enrich_extraction import EnrichExtractor
...
class DependencyValidator:
    ...
    def validate_step_3(self) -> bool:
        ...
        enriched_graph = self.project_root / self.config["graph_builder"]["input_path"]
...
class IndexingPipeline:
    ...
    def run_step_1_ocr_parsing(self):
        ...
        parser = VLMPdfParser(self.config)
        parser.upload_files()
    ...
    def run_step_2_enrich_extraction(self):
        ...
        extractor = EnrichExtractor(self.config)
        extractor.run()
    ...
    def run_step_3_graph_building(self):
        ...
        graph_builder_main(self.config)
...
执行示例:
  ...
  python run_indexing_qwen.py --start 3 --end 4 --ablation text_only  # 消融实验
    ...
    try:
        ...
        if args.step:
            ...
            if start_step > end_step:
                logging.info("✅ 指定范围的步骤已完成，无需继续执行")
                return
            logging.info(f"📍 从步骤 {start_step} 继续执行到步骤 {end_step} (上次完成: 步骤 {last_completed})")
        else:
            # 执行指定范围
